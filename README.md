...
phone/app         폰 에이전트 앱. Compose Material 3 UI
```
...
| capability `watchvoice_agent` | 폰이 광고 | 에이전트 설치/도달 가능 여부 판별 |
